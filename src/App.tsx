...
import AIFinancialAnalystView from './components/AIFinancialAnalystView';
...
export default function App() {
  ...
  const saveCampaigns = (updated: AdCampaign[]) => {
    ...
    localStorage.setItem('zynads_campaigns', JSON.stringify(updated));
  };
    ...
    <div id="app-root-container" className="flex flex-col md:flex-row h-screen w-screen bg-slate-50/60 overflow-hidden text-slate-800 font-sans">
      ...
      <header id="mobile-header" className="md:hidden flex items-center justify-between px-4 py-3 bg-slate-950 text-white border-b border-slate-800 shrink-0 z-40">
        <div id="mobile-brand-wrapper" className="flex items-center gap-2.5">
          ...
          >
            <div id="mobile-logo-icon" className="p-1.5 bg-gradient-to-tr from-teal-500 to-emerald-500 rounded-lg text-slate-950 flex items-center justify-center font-black">
              <Zap className="w-4 h-4 text-slate-950 fill-current" />
            </div>
            <div>
              <div className="font-bold text-sm leading-tight text-white flex items-center gap-1">
                Zyncast<span className="text-teal-400 font-black">CFO</span>
              </div>
              <span className="text-[9px] text-slate-400 font-mono block">EXECUTIVE SUITE</span>
            </div>
          </button>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded-lg text-[11px] font-extrabold flex items-center gap-1 transition-all cursor-pointer ${
              activeTab === 'video-studio' || activeTab === 'voiceovers' || activeTab === 'creatives' || activeTab === 'campaigns'
          ...
          <button
            ...
            className="p-2 bg-slate-900 hover:bg-slate-800 text-slate-200 hover:text-white border border-slate-700 rounded-xl transition-all cursor-pointer flex items-center justify-center shadow-md active:scale-95"
        ...
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="bg-slate-900/95 border border-slate-800 p-3.5 rounded-2xl shadow-xl flex flex-wrap items-center justify-between gap-3 text-white">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button
                onClick={() => setActiveTab('dashboard')}
                className={`px-3 py-1.5 rounded-xl text-xs font-extrabold transition-all cursor-pointer flex items-center gap-1.5 shadow-md ${
                  activeTab === 'dashboard'
                    ? 'bg-teal-500 text-slate-950 ring-2 ring-teal-300'
                    : 'bg-slate-800 hover:bg-slate-700 text-teal-300 border border-teal-500/40'
                }`}
              >
                <span>📊 Zyncast CFO</span>
              </button>
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-extrabold transition-all cursor-pointer flex items-center gap-1.5 shadow-md ${
                  activeTab === 'ai-financial-analyst'
